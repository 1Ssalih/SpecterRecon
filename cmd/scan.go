...
var scanCmd = &cobra.Command{
	Use:   "scan [target]",
	Short: "Tüm adımları (Discovery -> Port Scan -> Banner -> CVE Match -> Dir Fuzz -> Report) sırasıyla çalıştırır",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		startTime := time.Now()
		core.LogAudit("FULL_PIPELINE_START", target, fmt.Sprintf("ports=%s, threads=%d", portsFlag, threadsFlag), "SUCCESS")

		// Step 1: Host Discovery
		core.LogStep("Adım 1 / 6: Host Discovery")
		hosts, _ := modules.DiscoverHosts(target, nil, 2*time.Second, threadsFlag, fmt.Sprintf("%s/hosts.json", outputDirFlag))
		if len(hosts) == 0 {
		...
		core.LogStep("Adım 2 / 6: Port & Servis Taraması")
		...
		if len(openPorts) == 0 {
			...
			report := modules.BuildCompleteReport(target, hosts, nil, nil, nil, nil, time.Since(startTime).Seconds())
		...
		core.LogStep("Adım 3 / 6: Banner Grabbing & Versiyon Tespiti")
		...
		if !skipVulnFlag {
			core.LogStep("Adım 4 / 6: CVE & Zafiyet Eşleştirmesi")
		...
		if !skipDirfuzzFlag {
			core.LogStep("Adım 5 / 6: Web Dizin & Dosya Fuzzing")
		...
		// Step 6: Reporting
		core.LogStep("Adım 6 / 6: Raporlama")
		duration := time.Since(startTime).Seconds()
		report := modules.BuildCompleteReport(target, hosts, openPorts, services, vulns, findings, duration)
		_, _ = modules.GenerateHTMLReport(report, "", fmt.Sprintf("%s/report.html", outputDirFlag))
