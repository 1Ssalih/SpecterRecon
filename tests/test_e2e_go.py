"""
End-to-End integration test simulating a mock HTTP target and testing the compiled Go binary (specter-recon.exe).
"""
...
def test_go_pipeline():
    ...
    exe_path = os.path.abspath("specter-recon.exe")
    assert os.path.exists(exe_path), f"Binary not found: {exe_path}"

    cmd = [
        exe_path, "scan", "127.0.0.1",
        "-p", str(port),
        "-t", "10",
        "--authorized",
        "-o", "output"
    ]
    print(f"[*] Executing Go binary command: {' '.join(cmd)}")
    ...
    with open("output/services.json", "r", encoding="utf-8") as f:
        data = f.read()
        assert "Apache" in data or "2.4.49" in data
        print("[+] Verified services.json contains detected Apache v2.4.49 service.")

    with open("output/vulns.json", "r", encoding="utf-8") as f:
        data = f.read()
        assert "CVE-2021-41773" in data
        print("[+] Verified vulns.json contains matched CVE-2021-41773.")

    with open("output/dirs.json", "r", encoding="utf-8") as f:
        data = f.read()
        assert "admin" in data or ".env" in data
        print("[+] Verified dirs.json contains discovered endpoints (.env, admin, etc.).")

    with open("output/report.html", "r", encoding="utf-8") as f:
        data = f.read()
        assert "SpecterRecon" in data
        assert "CVE-2021-41773" in data
        print("[+] Verified report.html contains full Go engine report with CVEs and findings!")

    server.shutdown()
    print("[SUCCESS] All Go Engine End-to-End pipeline checks passed successfully!")
